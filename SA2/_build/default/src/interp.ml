...
let rec new_env : string list -> env ea_result  = fun fs ->
  let rec newenv fs = 
    match fs with 
    | [] -> EmptyEnv
    | h::t-> ExtendEnv(h, RefVal(Store.new_ref g_store (NumVal 0)), (newenv t))
  in 
    return (newenv (List.rev fs))
...
let lookup_method : string -> string -> class_env ->
  ...
  (match List.assoc_opt c_name c_env with
  	...
  	| Some (_,_,n) -> (List.assoc_opt m_name n))
...
and
  ...
  | NewObject(c_name,args) ->
    sequence (List.map eval_expr args) >>= fun arg ->
    ...
    | Some m ->
      apply_method "initialize" (ObjectVal (c_name, env)) arg m >>= fun _ ->
      return (ObjectVal(c_name, env)))
  | Send(e,m_name,args) ->
    eval_expr e >>= fun t -> obj_of_objectVal t >>= fun (c_name, _) ->
    sequence (List.map eval_expr args) >>= fun arg ->
    (match lookup_method c_name m_name !g_class_env with
    | None -> error "Method not found"
    | Some m -> apply_method m_name t arg m)
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,args) ->
    sequence (List.map eval_expr args) >>= fun arg ->
    ...
    (match lookup_method c_name m_name !g_class_env with
    | None -> error "Method not found"
    | Some m -> apply_method m_name self arg m)
